...
export function HospitalLoginPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE_BYTES) {
      setSuratTugasError(`Ukuran file maksimal ${MAX_FILE_SIZE_MB}MB. File Anda: ${(file.size / 1024 / 1024).toFixed(1)}MB`);
      return;
    }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!regHospitalName.trim()) {
      setError("Nama rumah sakit wajib diisi.");
      return;
    }
    ...
    if (!regPicName.trim()) {
      setError("Nama penanggung jawab wajib diisi.");
      return;
    }

    if (!suratTugasFile) {
      setError("Surat tugas (PDF) wajib diunggah.");
      return;
    }
    ...
    reader.onload = () => {
      ...
      if (ok) {
        setSuccess(
          `Registrasi berhasil untuk ${regHospitalName.trim()}! Akun Anda akan diaktivasi oleh admin PERSI. Silakan menunggu sebelum bisa login.`
        );
        setMode("login");
        ...
        <div className="text-white space-y-8 hidden lg:block">
          ...
          <div>
            ...
            <p className="text-lg text-white/80 leading-relaxed">
              ...
              assessment dan masuk ke ranking nasional.
            </p>
          </div>
          ...
          <div className="space-y-3">
            <InfoItem
              ...
              description="Rumah sakit dapat mendaftar langsung dengan mengisi data dan mengunggah surat tugas"
            />
            <InfoItem
              icon={<FileText className="w-4 h-4" />}
              title="Upload Surat Tugas"
              description="Surat tugas PIC wajib diunggah saat registrasi (PDF, maks 2MB)"
            />
            <InfoItem
              icon={<Clock className="w-4 h-4" />}
              title="Aktivasi oleh Admin"
              description="Setelah mendaftar, akun harus diaktifkan oleh admin PERSI sebelum bisa login"
            />
            <InfoItem
              icon={<CheckCircle2 className="w-4 h-4" />}
              title="Assessment Terstruktur"
              description="Isi data RSBK, Clinical Audit, dan Patient Report per spesialisasi"
            />
          </div>

          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/10">
            <p className="text-sm text-white/70">
              <strong className="text-white">Butuh bantuan?</strong> Hubungi
              admin PERSI di{" "}
              <a
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <div className="flex bg-gray-100 rounded-lg p-1 mb-6">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-md text-sm font-[600] transition-colors ${
                mode === "login"
          ...
          {mode === "login" ? (
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="mb-6">
                <h2 className="text-2xl font-[700] text-gray-900 mb-1">
                  Login Portal RS
                </h2>
                <p className="text-sm text-gray-500">
                  Masuk dengan akun rumah sakit yang sudah diaktivasi
                </p>
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
                <div className="flex items-start gap-2">
                  ...
                  <span>
                    Baru mendaftar? Akun harus diaktivasi oleh admin PERSI terlebih dahulu sebelum bisa login.
                  </span>
                </div>
              </div>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-500">
                  Isi data rumah sakit dan unggah surat tugas PIC
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="RS Harapan Sehat Jakarta"
                    className="pl-10 h-11"
                    required
                  />
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="Dr. Ahmad Sudrajat, Sp.PD"
                    className="pl-10 h-11"
                    required
                  />
                </div>
              ...
              {/* PDF Upload - Surat Tugas */}
              <div className="space-y-2">
                <Label className="text-sm font-[600] text-gray-700">
                  Upload Surat Tugas (PDF) *
                </Label>
                <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-colors ${
                  suratTugasFile ? "border-green-400 bg-green-50" : "border-gray-300 hover:border-[#1E3A8A] hover:bg-blue-50"
                }`}>
                  {suratTugasFile ? (
                    <div className="flex items-center gap-3 justify-center">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-500">
                          {(suratTugasFile.size / 1024).toFixed(0)} KB
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={() => setSuratTugasFile(null)}
                        className="text-red-500 text-xs font-[600] hover:underline ml-2"
                      >
                        Hapus
                      </button>
                    </div>
                    ...
                    <label className="cursor-pointer block">
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        Format: PDF | Maksimal: {MAX_FILE_SIZE_MB}MB
                      </p>
                      <input
                        type="file"
                ...
                {suratTugasError && (
                  <p className="text-xs text-red-600">{suratTugasError}</p>
                )}
              </div>
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full h-11 bg-[#0D9488] hover:bg-[#0b7f75] font-[600]"
              >
                {loading ? (
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
...
function InfoItem({
  ...
  return (
    <div className="flex items-start gap-3 bg-white/5 rounded-lg p-3 border border-white/10">
      <div className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center flex-shrink-0 text-white/80">
        {icon}
      </div>
      <div>
